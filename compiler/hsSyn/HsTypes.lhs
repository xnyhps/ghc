...
ppr_mono_ty _    (HsTupleTy con tys) = tupleParens con (interpp'SP tys)  -- UNDEFINED: we might want to differenciate a type from a kind to put a quote
ppr_mono_ty _    (HsKindSig ty kind) = parens (ppr_mono_lty pREC_TOP ty <+> dcolon <+> ppr kind)
ppr_mono_ty _    (HsListTy ty)	     = brackets (ppr_mono_lty pREC_TOP ty)  -- UNDEFINED: we might want to differenciate a type from a kind to put a quote
ppr_mono_ty _    (HsPArrTy ty)	     = pabrackets (ppr_mono_lty pREC_TOP ty)
ppr_mono_ty _    (HsPredTy pred)     = ppr pred
ppr_mono_ty _    (HsSpliceTy s _ _)  = pprSplice s
ppr_mono_ty _    (HsCoreTy ty)       = ppr ty
ppr_mono_ty _    (HsPromotedConTy name) = quote (ppr name)
ppr_mono_ty _    (HsLitTy lit)       = ppr lit  -- UNDEFINED: do we want quote here?
